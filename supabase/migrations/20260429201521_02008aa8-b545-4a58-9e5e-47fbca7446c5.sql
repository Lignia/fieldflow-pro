...
CREATE FUNCTION public.search_quote_items(
  p_tenant_id uuid,
  p_query text,
 ...
 RETURNS TABLE(id uuid, name text, normalized_name text, sku text, supplier_ref text, supplier_name text, supplier_range text, product_kind text, product_type text, technology_type text, diameter_inner_mm smallint, diameter_outer_mm smallint, length_mm smallint, angle_deg smallint, finish_color text, unit_price_ht numeric, cost_price numeric, vat_rate numeric, unit text, normalization_status text, normalization_confidence numeric, search_score integer, boost_score smallint, default_visible boolean)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    s.id, s.name, s.normalized_name, s.sku, s.supplier_ref, s.supplier_name,
